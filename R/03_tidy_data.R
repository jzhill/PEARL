...
min_week <- floor_date(
  min(screening_data$en_date_visit, na.rm = TRUE),
  unit = "week",
  week_start = 1
)
min_month <- floor_date(
  min(screening_data$en_date_visit, na.rm = TRUE),
  unit = "month"
)
max_week <- floor_date(
  max(screening_data$en_date_visit, na.rm = TRUE),
  unit = "week",
  week_start = 1
)
max_month <- floor_date(
  max(screening_data$en_date_visit, na.rm = TRUE),
  unit = "month"
)
...
screening_data <- screening_data %>%
  mutate(
    ...
    tbdec_comp_bin = !is.na(tb_decision) &
      tb_decision != "" &
      tb_decision != "Uncertain",
...
screening_data <- screening_data %>%
  mutate(
    xpert_available = case_when(
      ...
      en_date_visit >= as.Date("2023-04-17") &
        en_date_visit <= as.Date("2023-04-30") ~ FALSE,
      en_date_visit >= as.Date("2023-06-26") &
        en_date_visit <= as.Date("2023-08-06") ~ FALSE,
      en_date_visit >= as.Date("2023-08-14") &
        en_date_visit <= as.Date("2023-08-20") ~ FALSE,
      en_date_visit >= as.Date("2024-10-21") &
        en_date_visit <= as.Date("2024-12-15") ~ FALSE,
      en_date_visit >= as.Date("2025-01-20") &
        en_date_visit <= as.Date("2025-02-09") ~ FALSE,
      en_date_visit >= as.Date("2025-04-14") &
        en_date_visit <= as.Date("2025-07-20") ~ FALSE,
      en_date_visit >= as.Date("2025-07-28") &
        en_date_visit <= as.Date("2025-11-30") ~ FALSE,
...
screening_data <- screening_data %>%
  mutate(
    ...
    tb_moreinf_bin = case_when(
      tb_confirmed &
        (xpert_pos_ex_trace | (new_tb_case & xr_consistent_tb)) ~ TRUE,
      tb_confirmed ~ FALSE,
      TRUE ~ NA
    )
  )
...
screening_data <- screening_data %>%
  mutate(
    week_reg = floor_date(en_date_visit, unit = "week", week_start = 1)
  ) %>%
...
# TODO add a flag to indicate when household EA/village is used vs screening data EA/village
...
screening_data <- screening_data %>%
  mutate(
    ...
    tptrf_assessed = !is.na(calc_tptrfs_chr) &
      calc_tptrfs_chr != "06 RFs not complete",
    ...
    tptrf_risk_assigned = !is.na(prerx_riskcat_chr) &
      prerx_riskcat_chr != "Not yet known",
...
screening_data <- screening_data %>%
  mutate(
    ...
    tpt_inelig_reason = case_when(
      ...
      prerx_tbsx == TRUE |
        (tb_decision == "Presumptive TB" &
          ntp_diagnosis != "Ruled out") ~ "Presumed TB",
      prerx_tptchoice == FALSE |
        exit_reason_screen ==
          "TPT - withdraw consent and prefer not to continue" ~ "Refused",
    ...
    tpt_not_assessed_reason = case_when(
      ...
      (tst_read_positive == "Positive TST" &
        ...
        exit_reason_screen %in%
          c(
            "TPT - lost to follow-up",
            "Screening - lost to follow-up"
          ) ~ "LTFU",
...
treatment_data <- treatment_data %>%
  mutate(
    ...
    tpt_3m_complete_date = if_else(
      tpt_3m_outcome == "Complete TPT",
      tpt_3m_date,
      as.Date(NA)
    ),
    tpt_4m_complete_date = if_else(
      tpt_4m_outcome == "Complete TPT",
      tpt_4m_date,
      as.Date(NA)
    ),

    # --- REAL end date: earliest of outcome/complete dates; NA if none exist
    .tpt_end_real_num = do.call(
      pmin,
      c(
    ...
    tpt_end_date_real = as.Date(
      ifelse(is.infinite(.tpt_end_real_num), NA, .tpt_end_real_num),
      origin = "1970-01-01"
    ),

    # PROVISIONAL end date: real end if known, else today()
    # Note that this will change over time as today() changes, so it is not a fixed historical value
    tpt_end_date_prov = coalesce(tpt_end_date_real, today()),

    # Durations
    # Real duration - duration on TPT for those completed, is NA if still active (no real end date)
    tpt_dur_real = as.integer(difftime(
      tpt_end_date_real,
      tpt_start_date,
      units = "days"
    )),
    # Provisional duration - duration on TPT whether completed or still active
    tpt_dur_prov = as.integer(difftime(
      tpt_end_date_prov,
      tpt_start_date,
      units = "days"
    )),
    ...
    # This flags for each participant whether they have been on TPT long enough to have been expected to have completed the 1m, 3m, or 4m review
    # thresholds ≈ 1, 3, 4 months
    # Allows us to measure performace by comparing expected vs done, and to identify those who are overdue for review
...
screen_core_by_key <- function(df_keyed) {
  df_keyed %>%
    ...
    summarise(
      ...
      tst_read_pct = if_else(
        tst_placed > 0,
        100 * tst_read / tst_placed,
        NA_real_
      ),
      ...
      cxr_res_pct = if_else(
        cxr_done > 0,
        100 * cxr_result / cxr_done,
        NA_real_
      ),
      ntp_out_pct = if_else(
        ref_ntp > 0,
        ntp_outcome_recorded / ref_ntp * 100,
        NA_real_
      ),
      nlp_out_pct = if_else(
        ref_nlp > 0,
        nlp_outcome_recorded / ref_nlp * 100,
        NA_real_
      ),
      .groups = "drop"
    )
...
tb_dist_by_key <- function(df_keyed) {
  df_keyed %>%
    mutate(
      tb_decision = fct_na_value_to_level(factor(tb_decision), "Missing")
    ) %>%
...
tst_dist_by_key <- function(df_keyed) {
  df_keyed %>%
    mutate(
      tst_read_positive = fct_na_value_to_level(
        factor(tst_read_positive),
        "Missing"
      )
    ) %>%
    count(.key, tst_read_positive, name = "n") %>%
    pivot_wider(
      names_from = tst_read_positive,
      values_from = n,
      values_fill = 0
    ) %>%
    rename(
  ...
  df_keyed %>%
    ...
    summarise(
      ...
      tpt_eligible_of_started_pct = if_else(
        tpt_started_n > 0,
        100 *
          sum(prerx_eligible == "Yes" & prerx_start == TRUE, na.rm = TRUE) /
          tpt_started_n,
        NA_real_
      ),
...
tpt_cohort_by_key <- function(tpt_keyed) {
  tpt_keyed %>%
    ...
    summarise(
      ...
      tpt_outcome_assigned_pct = if_else(
        tpt_start > 0,
        100 * tpt_outcome_assigned / tpt_start,
        NA_real_
      ),
      tpt_completed_pct = if_else(
        tpt_start > 0,
        100 * tpt_completed / tpt_start,
        NA_real_
      ),
      tpt_1m_done_pct = if_else(
        tpt_1m_expected > 0,
        100 * tpt_1m_done / tpt_1m_expected,
        NA_real_
      ),
      tpt_3m_done_pct = if_else(
        tpt_3m_expected > 0,
        100 * tpt_3m_done / tpt_3m_expected,
        NA_real_
      ),
      tpt_4m_done_pct = if_else(
        tpt_4m_expected > 0,
        100 * tpt_4m_done / tpt_4m_expected,
        NA_real_
      ),
      .groups = "drop"
    )
...
hh_targets_by_key <- function(hh_keyed) {
  hh_keyed %>%
    ...
    summarise(
      ...
      target_non_eligible = sum(as.numeric(hh_size), na.rm = TRUE) -
        sum(hh_size_elig, na.rm = TRUE),
      .groups = "drop"
    )
}
...
census_denoms_ea <- function(ea_data) {
  ea_data %>%
    transmute(
      .key = record_id,
      # Denominators: 2020, 2023, and PEARL project totals
      pop_2020,
      pop_2020_m,
      pop_2020_f,
      hh_2020,
...
census_denoms_village <- function(ea_data) {
  ea_data %>%
    # Ensure all villages have a label to prevent NA-grouping collisions
    mutate(village = coalesce(village, "Unreached / Unknown")) %>%
    group_by(village) %>%
    summarise(
      # 2020 Census
      pop_2020 = sum(pop_2020, na.rm = TRUE),
      pop_2020_m = sum(pop_2020_m, na.rm = TRUE),
      pop_2020_f = sum(pop_2020_f, na.rm = TRUE),
      hh_2020 = sum(hh_2020, na.rm = TRUE),
      # PEARL enumeration
      pop_all = sum(pop_all, na.rm = TRUE),
      pop_current = sum(pop_current, na.rm = TRUE),
      pop_elig = sum(pop_elig, na.rm = TRUE),
      pop_reg_enum = sum(pop_reg_enum, na.rm = TRUE),
      # 2023 hh survey
      pop_2023 = sum(pop_2023, na.rm = TRUE),
      pop_2023_m = sum(pop_2023_m, na.rm = TRUE),
      pop_2023_f = sum(pop_2023_f, na.rm = TRUE),
...
build_time_agg <- function(
  ...
  # bounds
  min_key <- if (freq == "week") {
    min(screening_data$week_reg, na.rm = TRUE)
  } else {
    min(screening_data$month_reg, na.rm = TRUE)
  }
  max_key <- if (freq == "week") {
    max(screening_data$week_reg, na.rm = TRUE)
  } else {
    max(screening_data$month_reg, na.rm = TRUE)
  }
  ...
  full_seq <- if (freq == "week") {
    seq.Date(
      from = floor_date(min_key, "week", week_start = week_start),
      to = floor_date(max_key, "week", week_start = week_start),
      by = "week"
    )
  } else {
    seq.Date(
      from = floor_date(min_key, "month"),
  ...
  out <- tibble(.key = full_seq) %>%
    ...
    zero_fill_counts(c(
      ...
      "ref_nlp",
      "ref_hbv",
      "nlp_outcome_recorded",
      "ntp_outcome_recorded",
      "hh_enum_new",
      "tpt_start",
      "tpt_completed",
      ...
      "tptrf_assessed_n",
      "tptrf_alt_needed_n",
      "tptrf_alt_requested_n",
      "tptrf_alt_result_n",
      "tptrf_risk_assigned_n",
      "tptrf_as_expected_n"
    ))
...
build_area_agg <- function(
  ...
  if (level == "ea") {
    ...
    # Use coalesced village labels to match the skeleton
    scr_keyed <- screening_data %>%
      mutate(village = coalesce(village, "Unreached / Unknown")) %>%
      filter(village != "") %>%
      mutate(.key = village)
    ...
    hh_keyed <- household_data %>%
      mutate(hh_village_ea = coalesce(hh_village_ea, "Unreached / Unknown")) %>%
      filter(hh_village_ea != "") %>%
      mutate(.key = hh_village_ea)

    tpt_keyed <- treatment_data %>%
      left_join(
        screening_data %>% select(record_id, village),
        by = "record_id"
      ) %>%
      mutate(village = coalesce(village, "Unreached / Unknown")) %>%
      filter(village != "") %>%
  ...
  out <- den_census %>%
    ...
    arrange(.key) %>%
    rename(area = .key) %>%
    mutate(target_missed = pmax(0, target_eligible - reg)) %>%
    zero_fill_counts(c(
      "pop_2020",
      "pop_2020_m",
      "pop_2020_f",
...
build_teams_agg <- function(screening_data, household_data, treatment_data) {
  ...
  tpt_prep <- treatment_data %>%
    left_join(
      screening_data %>% select(record_id, en_team),
      by = "record_id"
    ) %>%
...
ea_agg_bundle <- build_area_agg(
  screening_data,
  household_data,
  treatment_data,
  level = "ea"
)
ea_agg_wide <- ea_agg_bundle$data
ea_agg_long <- ea_agg_bundle$data_long

# Identify dynamic metric columns for idempotent join
ea_metric_cols <- setdiff(
  names(ea_agg_wide),
  c(
    "area",
    "pop_2020",
...
# Consolidate into the master ea_data object
ea_data <- ea_data %>%
  select(-any_of(ea_metric_cols)) %>%
  left_join(
    ea_agg_wide %>% select(area, all_of(ea_metric_cols)),
    by = c("record_id" = "area")
  )
...
# 1. Generate the Village Skeleton from ea_data
village_skeleton <- ea_data %>%
  mutate(village = coalesce(village, "Unreached / Unknown")) %>%
  group_by(village) %>%
  summarise(
    # The first 3 digits represent the Island + Village code for census order
    village_code = min(as.numeric(str_sub(record_id, 1, 3)), na.rm = TRUE),
    across(
      any_of(c(
        "pop_2020",
        "pop_2020_m",
...
# 2. Generate metrics
village_agg_bundle <- build_area_agg(
  screening_data,
  household_data,
  treatment_data,
  level = "village"
...
village_metrics <- village_agg_bundle$data %>%
  rename(village = area) %>%
  mutate(village = coalesce(village, "Unreached / Unknown"))

# 3. Identify metric-only columns
vil_metric_cols <- setdiff(names(village_metrics), names(village_skeleton))
vil_fill_cols <- vil_metric_cols[
  !grepl("date", vil_metric_cols, ignore.case = TRUE)
]

# 4. Final join and Factor Ordering
village_data <- village_skeleton %>%
  left_join(
    village_metrics %>% select(village, all_of(vil_metric_cols)),
    by = "village"
  ) %>%
  zero_fill_counts(vil_fill_cols) %>%
  arrange(village_code) %>%
  # Lock in the Census Factor Levels
  mutate(village = factor(village, levels = unique(village)))

# 5. Create long format for indicators
village_agg_long <- village_data %>%
  mutate(across(-village, as.character)) %>%
  pivot_longer(-village, names_to = "Indicator", values_to = "Value")


## Cumulative village data -----------------------------------------

# 1. Identify villages with total registrations >= 100 for focused plotting
villages_gte_100 <- village_data %>%
  filter(reg >= 100) %>%
  pull(village) %>%
  as.character()

# 2. Define the global week range for the project
...
# 3. Build cumulative time series
village_data_cum <- screening_data %>%
  mutate(
    # Collapse small/unreached villages into "Other/Unreached" for cleaner lines
    village_gte_100 = if_else(
      as.character(village) %in% villages_gte_100,
      as.character(village),
      "Other or Unreached"
    )
  ) %>%
  filter(!is.na(week_reg)) %>%
  group_by(village_gte_100, week_reg) %>%
  summarise(
    n_reg = n(),
    n_complete = sum(tbdec_comp_bin, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  # Ensure continuous lines for all selected villages
  complete(
    village_gte_100,
    week_reg = week_range_complete,
    fill = list(n_reg = 0, n_complete = 0)
  ) %>%
  group_by(village_gte_100) %>%
  mutate(
    cum_reg = cumsum(n_reg),
    cum_complete = cumsum(n_complete)
  ) %>%
  ungroup() %>%
  # 4. Join denominators and sync factor levels
  left_join(
    village_data %>%
      mutate(village_gte_100 = as.character(village)) %>%
      select(village_gte_100, pop_2023_ex02, pop_elig_new, first_screen_date),
    by = "village_gte_100"
  ) %>%
  mutate(
    # Sync with master village_data order; "Other" will naturally go to the end
    village_gte_100 = factor(
      village_gte_100,
      levels = c(levels(village_data$village), "Other or Unreached")
    ),
    cum_prop_2023_ex02 = cum_reg / pop_2023_ex02,
    cum_prop_elig_new = cum_reg / pop_elig_new
  )
...
layer_ki_ea_3832 <- layer_ki_ea_3832 %>%
  ...
  rename_with(
    ~ paste0("joined_", .),
    .cols = any_of(setdiff(names(ea_data), "record_id"))
  ) # Rename only newly added columns

# Filter to only include EAs with vid of 716
layer_betio_ea_3832 <- layer_ki_ea_3832 %>%
  filter(vid == 716)

# Filter to only include EAs with iid of 7
layer_st_ea_3832 <- layer_ki_ea_3832 %>%
  filter(iid == 7)
...
tidy_data <- list(
  ...
  # Master Geographic Data (Consolidated)
  ea_data = ea_data,
  village_data = village_data,

  # Long format data for plotting
  ea_long = ea_agg_long,
  village_long = village_agg_long,
  ...
  # Village specific progression
  village_data_cum = village_data_cum,
  village_order_cum = levels(village_data$village),

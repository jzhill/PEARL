...
kiribati_ea$prop_reg[is.na(kiribati_ea$prop_reg)] <- 0
...
kiribati_ea <- kiribati_ea %>%
  mutate(fill_category_prop_reg = ifelse(prop_reg < 0.2, "Below 20pc", "Above 20pc"))

# Create labels with EA ID and proportion
kiribati_ea <- kiribati_ea %>%
  mutate(label_text = paste0(ea_2020, "\n", round(prop_reg * 100, 1), "%"))

# Plot map
ggplot(kiribati_ea) +
  geom_sf(aes(fill = prop_reg), color = "black", size = 0.2) +

...
layer_betio_ea_3832 <- layer_betio_ea_3832 %>%
  mutate(fill_category_prop_reg = ifelse(joined_prop_reg_screen < 0.2, "Below 20pc", "Above 20pc"))

# Create labels with EA ID and proportion
layer_betio_ea_3832 <- layer_betio_ea_3832 %>%
  mutate(label_text = paste0(ea_2020, "\n", round(joined_prop_reg_screen * 100, 1), "%"))

# Plot map
map_05.07 <- ggplot(layer_betio_ea_3832) +
  geom_sf(aes(fill = joined_prop_reg_screen), color = "black", size = 0.2) +
